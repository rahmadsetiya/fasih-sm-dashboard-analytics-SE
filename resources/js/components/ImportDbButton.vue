...
function uploadFile(file: File) {
    ...
    xhr.onload = () => {
        ...
        if (xhr.status === 200) {
            status.value = { exists: true, size_mb: data.size_mb, modified_at: data.modified_at };
    ...
    <Dialog v-model:open="open">
        ...
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                ...
                <DialogDescription>
                    Upload file <code class="rounded bg-muted px-1 py-0.5 text-xs">fasih.db</code> untuk memperbarui data dashboard.
                </DialogDescription>
            </DialogHeader>
            ...
            >
                <component :is="status.exists ? CheckCircle2 : AlertCircle" class="mt-0.5 size-4 shrink-0" />
                <div class="space-y-0.5">
                    <p class="font-medium">
                        {{ status.exists ? 'Database aktif' : 'Belum ada database' }}
                    </p>
                    <p v-if="status.exists" class="text-xs opacity-80">
                        {{ status.size_mb }} MB &mdash; diperbarui {{ fmtDate(status.modified_at) }}
                    </p>
            ...
            >
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">atau klik untuk memilih file (.db, .sqlite)</p>
                </div>
