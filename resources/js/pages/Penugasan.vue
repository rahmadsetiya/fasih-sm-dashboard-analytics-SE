...
interface Assignment {
    ...
    status: string; assignment_status_id: number;
    date_modified: string; nmkec: string | null; kdkec: string; nmdes: string | null; kddes: string;
}
interface HistoryRow {
    id: number; from_status: string | null; to_status: string;
    change_date: string; pencacah_email: string | null; pengawas_email: string | null;
}
...
<template>
    ...
    <div v-else class="space-y-3 p-4">
        ...
        <div class="flex flex-wrap items-end gap-2">
            <div class="flex flex-col gap-1">
                ...
                <select v-model="filterStatus" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option v-for="s in statuses" :key="s.id" :value="s.id">{{ s.label }}</option>
                </select>
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">Kecamatan</label>
                <select v-model="filterKec" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option value="">Semua</option>
                    <option v-for="k in kecOptions" :key="k.code" :value="k.code">{{ k.name }}</option>
                </select>
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">Desa</label>
                <select v-model="filterDes" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option value="">Semua</option>
                    <option v-for="d in desOptions" :key="d.code" :value="d.code">{{ d.name }}</option>
                </select>
            </div>
            <div v-if="filterDes" class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">SLS</label>
                <select v-model="filterSls" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option value="">Semua</option>
                    <option v-for="s in slsOptions" :key="s.code" :value="s.code">{{ s.name }}</option>
                </select>
            </div>
            <div v-if="filterSls" class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">Sub-SLS</label>
                <select v-model="filterSubsls" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option value="">Semua</option>
                    <option v-for="ss in subslsOptions" :key="ss.code" :value="ss.code">{{ ss.name }}</option>
                </select>
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">Pengawas</label>
                <select v-model="filterPengawas" class="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
                    <option value="">Semua</option>
                    <option v-for="p in pengawas_options" :key="p.email" :value="p.email">{{ p.nama }}</option>
                </select>
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-[10px] font-medium text-muted-foreground">Cari Kode</label>
                <input v-model="search" type="text" placeholder="Kode penugasan..."
                    class="h-8 w-44 rounded-md border border-input bg-background px-2 text-xs placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                    @keydown.enter="applyFilters" />
            </div>
            <button class="h-8 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors" @click="applyFilters">Terapkan</button>
            <button class="h-8 rounded-md border border-input bg-background px-3 text-xs text-muted-foreground hover:text-foreground transition-colors"
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <select v-model="perPage" class="h-7 rounded border border-input bg-background px-1 text-xs">
        ...
        <div class="overflow-x-auto rounded-lg border">
            <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-border">
                    <tr v-if="loading"><td colspan="7" class="py-8 text-center text-muted-foreground">Memuat data...</td></tr>
                    <template v-else>
                        <tr v-for="(row, i) in rows" :key="row.assignment_id"
                            class="cursor-pointer hover:bg-muted/40" @click="openHistory(row)">
                            <td class="px-3 py-2 text-muted-foreground">{{ (page-1)*perPage+i+1 }}</td>
                            <td class="px-3 py-2 font-mono text-[10px]">{{ row.code_identity }}</td>
                            <td class="px-3 py-2"><div>{{ row.pencacah_nama ?? '—' }}</div><div class="text-[10px] text-muted-foreground">{{ row.pencacah_email ?? '' }}</div></td>
                            <td class="px-3 py-2 text-muted-foreground">{{ row.pengawas_nama ?? '—' }}</td>
                            <td class="px-3 py-2"><div>{{ row.nmkec ?? row.kdkec }}</div><div class="text-[10px] text-muted-foreground">{{ row.nmdes ?? row.kddes }}</div></td>
                            <td class="px-3 py-2"><span :class="['rounded px-1.5 py-0.5 font-medium', STATUS_BADGE[row.assignment_status_id] ?? 'bg-gray-100 text-gray-700']">{{ row.status }}</span></td>
                            <td class="px-3 py-2 text-muted-foreground">{{ fmtDate(row.date_modified) }}</td>
                        </tr>
                        <tr v-if="!rows.length"><td colspan="7" class="py-8 text-center text-muted-foreground">Tidak ada penugasan ditemukan</td></tr>
                    </template>
                </tbody>
            </table>
        </div>
        ...
        <div v-if="totalPages > 1" class="flex items-center justify-between text-xs text-muted-foreground">
            <span>{{ ((page-1)*perPage+1).toLocaleString('id') }}–{{ Math.min(page*perPage,total).toLocaleString('id') }} dari {{ total.toLocaleString('id') }}</span>
            <div class="flex items-center gap-1">
                <button :disabled="page===1" class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40" @click="page--">‹</button>
    ...
    <!-- History Dialog -->
    <Teleport to="body">
        <div v-if="historyOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="historyOpen = false">
            <div class="w-full max-w-lg rounded-xl border bg-background shadow-xl">
                <div class="flex items-start justify-between border-b px-4 py-3">
                    <div>
                        ...
                        <p class="font-mono text-[10px] text-muted-foreground">{{ selectedRow?.code_identity }}</p>
                        <p class="mt-0.5 text-xs text-muted-foreground">{{ selectedRow?.pencacah_nama ?? '' }}</p>
                    </div>
                    <button class="mt-0.5 rounded-md p-1 text-muted-foreground hover:bg-muted" @click="historyOpen = false">✕</button>
                </div>
                <div class="max-h-96 overflow-y-auto p-4">
                    <div v-if="historyLoading" class="flex h-24 items-center justify-center text-sm text-muted-foreground">Memuat...</div>
                    <div v-else-if="!historyRows.length" class="py-6 text-center text-sm text-muted-foreground">Tidak ada history</div>
                    <ol v-else class="relative border-l border-border pl-5">
                        <li v-for="h in historyRows" :key="h.id" class="mb-5 ml-1">
                            <span class="absolute -left-1.5 mt-0.5 h-3 w-3 rounded-full border-2 border-background bg-primary" />
                            <div class="text-xs">
                                <div class="flex items-center gap-1.5 font-medium">
                                    <span v-if="h.from_status" class="text-muted-foreground line-through">{{ h.from_status }}</span>
                                ...
                                <p v-if="h.pencacah_email" class="text-[10px] text-muted-foreground">Pencacah: {{ h.pencacah_email }}</p>
                                <p v-if="h.pengawas_email" class="text-[10px] text-muted-foreground">Pengawas: {{ h.pengawas_email }}</p>
                            </div>
                        </li>
                    </ol>
                </div>
