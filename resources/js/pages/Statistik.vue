...
interface BangunanKosongSummary {
    kategori: string;
    total: number;
    draft: number;
    submitted: number;
...
interface BangunanKosongKec {
    nmkec: string;
    kdkec: string;
    bangunan_kosong: number;
    rumah_kosong: number;
    total: number;
    submitted: number;
    approved: number;
...
const activeTab = ref<
    'proporsi' | 'komparasi' | 'chi2' | 'korelasi' | 'bangunan_kosong'
>('proporsi');
const loading = ref(false);

const proporsiGroup = ref<'kecamatan' | 'gelombang' | 'kelas' | 'tc'>(
    'kecamatan',
);
const proporsiData = ref<ProporsiRow[]>([]);

const kompGroupBy = ref<'kecamatan' | 'gelombang' | 'kelas' | 'tc'>(
    'kecamatan',
);
...
async function fetchProporsi() {
    ...
    try {
        const r = await fetch(
            `/api/statistik/proporsi?group_by=${proporsiGroup.value}`,
            { headers: H },
        );
...
async function runKomparasi() {
    ...
    try {
        const p = new URLSearchParams({
            group_by: kompGroupBy.value,
            group_a: kompGroupA.value,
            group_b: kompGroupB.value,
        });
...
async function fetchChi2() {
    ...
    try {
        const r = await fetch(
            `/api/statistik/chi2?group_by=${chi2GroupBy.value}`,
            { headers: H },
        );
...
const proporsiChartOptions = computed(() => ({
    ...
    xaxis: {
        categories: proporsiData.value.map((r) => r.group_label),
        labels: { style: { colors: cc.value, fontSize: '10px' }, rotate: -30 },
    },
    yaxis: {
        max: 100,
        labels: {
            formatter: (v: number) => `${v}%`,
            style: { colors: cc.value },
        },
    },
...
const proporsiSeries = computed(() => [
    { name: 'Approved %', data: proporsiData.value.map((r) => r.p_approved) },
    {
        ...
        data: proporsiData.value.map((r) => r.margin_of_error),
    },
]);
...
const korelasiChartOptions = computed(() => ({
    ...
    theme: { mode: (isDark.value ? 'dark' : 'light') as 'dark' | 'light' },
    xaxis: {
        title: { text: 'Avg Error', style: { color: cc.value } },
        labels: { style: { colors: cc.value } },
    },
    yaxis: {
        title: { text: 'Rejection Rate (%)', style: { color: cc.value } },
        labels: { style: { colors: cc.value } },
    },
    colors: ['#6366f1'],
    grid: { borderColor: isDark.value ? '#374151' : '#e5e7eb' },
...
const korelasiSeries = computed(() => [
    {
        name: 'Pencacah',
        data: (korelasiResult.value?.points ?? []).map((p) => [
            p.avg_error,
            p.rejection_rate,
        ]),
    },
    ...
    <div v-else class="space-y-4 p-4">
        <div>
            ...
            <p class="text-xs text-muted-foreground">
                Analisis statistik dari data penugasan sensus
            </p>
        </div>
        ...
        <template v-else-if="activeTab === 'proporsi'">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <button
                    ...
                    @click="proporsiGroup = g.id as typeof proporsiGroup"
                >
                    {{ g.label }}
                </button>
            </div>
            <div
                class="rounded-lg border bg-blue-50/50 p-3 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300"
            >
                <strong>Metode:</strong> Proporsi assignment Approved + interval
                kepercayaan 95% (1.96 × √(p(1−p)/n)).
            </div>
            <div v-if="proporsiData.length" class="rounded-lg border p-3">
                <VueApexCharts
                    type="bar"
                    :height="240"
                    :options="proporsiChartOptions"
                    :series="proporsiSeries"
                />
            </div>
        ...
        <template v-else-if="activeTab === 'komparasi'">
            <div
                class="rounded-lg border bg-blue-50/50 p-3 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300"
            >
                <strong>Metode:</strong> Uji Z dua proporsi independen (H₀: p₁ =
                p₂). Signifikan jika p-value &lt; 0.05.
            </div>
            <div class="flex flex-wrap items-end gap-3">
                ...
                <button
                    :disabled="!kompGroupA || !kompGroupB"
                    class="h-8 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
                    @click="runKomparasi"
                >
                    Hitung
                </button>
            ...
            <div v-if="kompResult">
                <div
                    v-if="kompResult.error"
                    class="rounded-lg border border-red-300 bg-red-50 p-3 text-xs text-red-700 dark:bg-red-950/30 dark:text-red-300"
                >
                    {{ kompResult.error }}
                </div>
                <div v-else class="space-y-3">
                    <div
                        :class="[
                            ...
                            boxClass(kompResult.significant),
                        ]"
                    >
                        <p
                            :class="[
                                'text-sm font-semibold',
                                boxTitleClass(kompResult.significant),
                            ]"
                        >
                            {{
                                kompResult.significant
                                    ? '✅ Signifikan'
                                    : '⚠️ Tidak Signifikan'
                            }}
                        </p>
                        <p class="mt-1 text-xs opacity-80">
                            {{ kompResult.interpretasi }}
                        </p>
                        <div class="mt-2 flex gap-6 text-xs">
                            <span
                                ><span class="opacity-70">Z:</span>
                                <span class="font-mono font-medium">{{
                                    kompResult.z
                                }}</span></span
                            >
                            <span
                                ><span class="opacity-70">p-value:</span>
                                <span class="font-mono font-medium">{{
                                    kompResult.p_value
                                }}</span></span
                            >
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div
                            v-for="g in ['group_a', 'group_b'] as const"
                            :key="g"
                        ...
                        >
                            ...
                            <p class="mt-1 text-xs text-muted-foreground">
                                n={{ kompResult[g].n.toLocaleString('id') }},
                                Approved={{
                                    kompResult[g].approved.toLocaleString('id')
                                }}
                                ({{ kompResult[g].p }}%)
                            </p>
                        </div>
        ...
        <template v-else-if="activeTab === 'chi2'">
            <div
                class="rounded-lg border bg-blue-50/50 p-3 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300"
            >
                <strong>Metode:</strong> Uji χ² untuk asosiasi antara variabel
                grup dan distribusi status. Signifikan jika p-value &lt; 0.05.
            </div>
            <div class="flex gap-2">
                <button
                    ...
                    @click="chi2GroupBy = g.id as typeof chi2GroupBy"
                >
                    {{ g.label }}
                </button>
            </div>
            ...
            <div v-if="chi2Result">
                ...
                <div v-else class="space-y-3">
                    <div
                        :class="[
                            'rounded-lg border p-3',
                            boxClass(chi2Result.significant),
                        ]"
                    >
                        <p
                            :class="[
                                'text-sm font-semibold',
                                boxTitleClass(chi2Result.significant),
                            ]"
                        >
                            {{
                                chi2Result.significant
                                    ? '✅ Signifikan'
                                    : '⚠️ Tidak Signifikan'
                            }}
                        </p>
                        <p class="mt-1 text-xs opacity-80">
                            {{ chi2Result.interpretasi }}
                        </p>
                        <div class="mt-2 flex gap-6 text-xs">
                            <span
                                ><span class="opacity-70">χ²:</span>
                                <span class="font-mono font-medium">{{
                                    chi2Result.chi2
                                }}</span></span
                            >
                            <span
                                ><span class="opacity-70">df:</span>
                                <span class="font-mono font-medium">{{
                                    chi2Result.df
                                }}</span></span
                            >
                        ...
                        <table class="w-full text-xs">
                            <thead class="bg-muted/50">
                                <tr>
                                    ...
                                    <th
                                        v-for="label in chi2Result.col_labels"
                                        :key="label"
                                        class="px-3 py-2 text-right font-medium text-muted-foreground"
                                    >
                                        {{ label }}
                            ...
                            <tbody class="divide-y divide-border">
                                ...
                                >
                                    ...
                                    <td
                                        v-for="col in chi2Result.col_keys"
                                        :key="col"
        ...
        <template v-else-if="activeTab === 'korelasi'">
            <div
                class="rounded-lg border bg-blue-50/50 p-3 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300"
            >
                <strong>Metode:</strong> Korelasi Pearson (r) antara rata-rata
                sum_error dan tingkat penolakan per pencacah (min. 5 penugasan).
            </div>
            <div v-if="korelasiResult">
                <div
                    v-if="korelasiResult.error"
                    class="rounded-lg border border-red-300 bg-red-50 p-3 text-xs text-red-700"
                >
                    {{ korelasiResult.error }}
                </div>
                <div v-else class="space-y-3">
                    <div
                        :class="[
                            'rounded-lg border p-3',
                            boxClass(korelasiResult.significant),
                        ]"
                    >
                        <p
                            :class="[
                                'text-sm font-semibold',
                                boxTitleClass(korelasiResult.significant),
                            ]"
                        >
                            {{
                                korelasiResult.significant
                                    ? '✅ Signifikan'
                                    : '⚠️ Tidak Signifikan'
                            }}
                        </p>
                        <p class="mt-1 text-xs opacity-80">
                            {{ korelasiResult.interpretasi }}
                        </p>
                        <div
                            v-if="korelasiResult.r !== null"
                            class="mt-2 flex flex-wrap gap-6 text-xs"
                        >
                            <span
                    ...
                    <div
                        v-if="korelasiResult.points.length"
                        class="rounded-lg border p-3"
                    >
                        <p class="mb-2 text-xs text-muted-foreground">
                            Scatter: Avg Error (X) vs Rejection Rate % (Y)
                        </p>
                        <VueApexCharts
                            type="scatter"
                            :height="280"
                            :options="korelasiChartOptions"
                            :series="korelasiSeries"
                        />
                    </div>
                    <div class="overflow-x-auto rounded-lg border">
                        <table class="w-full text-xs">
                            ...
                            <tbody class="divide-y divide-border">
                                <tr
                                    v-for="p in korelasiResult.points"
                                    :key="p.nama"
                                    class="hover:bg-muted/30"
                                >
        ...
        <template v-else-if="activeTab === 'bangunan_kosong'">
            <p class="text-xs text-muted-foreground">
                Penugasan dengan tipe bangunan
                <code class="rounded bg-muted px-1">BANGUNAN KOSONG</code> atau
                <code class="rounded bg-muted px-1">RUMAH KOSONG</code>
                berdasarkan kolom
                <code class="rounded bg-muted px-1">data1</code>.
            </p>

            <!-- Summary cards -->
            <div class="grid grid-cols-2 gap-3">
                <div
                    v-for="row in bangunanSummary"
                    :key="row.kategori"
                ...
                >
                    ...
                    >
                        <div
                            ...
                            :style="{
                                width: `${row.total > 0 ? Math.round(((row.approved + row.submitted) / row.total) * 100) : 0}%`,
                            }"
                        />
                    </div>
                ...
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        >
                            ...
                            <td class="px-3 py-2 text-right">
                                {{ row.bangunan_kosong.toLocaleString('id') }}
                            </td>
                            <td class="px-3 py-2 text-right">
                                {{ row.rumah_kosong.toLocaleString('id') }}
                            </td>
                            <td class="px-3 py-2 text-right font-medium">
                                {{ row.total.toLocaleString('id') }}
                            </td>
