...
const pencacahPages = computed(() =>
    Math.ceil(pencacahAll.value.length / pencacahPer.value),
);
...
const turnaroundPages = computed(() =>
    Math.ceil(turnaroundData.value.length / turnaroundPer.value),
);
...
const sortedQuality = computed(() =>
    [...qualityAll.value].sort(
        (a, b) =>
            (b[qualitySortKey.value] as number) -
            (a[qualitySortKey.value] as number),
    ),
);
...
const qualityPages = computed(() =>
    Math.ceil(qualityAll.value.length / qualityPer.value),
);
...
const matrixPages = computed(() =>
    Math.ceil(matrixData.value.length / matrixPer.value),
);
...
const leaderboardData = computed<LeaderboardRow[]>(() =>
    [...pencacahAll.value]
        ...
        .map((r) => ({
            ...
            score: Math.round(
                r.progress_pct * 0.5 +
                    Math.max(0, 100 - r.rejection_rate) * 0.3 +
                    (r.total > 0 ? (r.approved / r.total) * 100 : 0) * 0.2,
            ),
        }))
        .sort((a, b) => b.score - a.score)
        .map((r, i) => ({ rank: i + 1, ...r })),
);
const leaderboardPages = computed(() =>
    Math.ceil(leaderboardData.value.length / leaderboardPer.value),
);
...
async function fetchTab(tab: typeof activeTab.value, force = false) {
    ...
    try {
        if (tab === 'pencacah') {
            if (!force && pencacahAll.value.length) {
                loading.value = false;

                return;
            }

            const r = await fetch(`/api/petugas/list?${geoParams()}`, {
                headers: H,
            });
            ...
            if (!force && turnaroundPencacah.value.length) {
                loading.value = false;

                return;
            }

            const r = await fetch(`/api/petugas/turnaround?${geoParams()}`, {
                headers: H,
            });
            const d = await r.json();
            ...
            if (!force && qualityAll.value.length) {
                loading.value = false;

                return;
            }

            const r = await fetch(`/api/petugas/quality?${geoParams()}`, {
                headers: H,
            });
            const d = await r.json();
            ...
            if (!force && gelombangList.value.length) {
                loading.value = false;
            ...
            if (!force && matrixData.value.length) {
                loading.value = false;

                return;
            }

            const r = await fetch(`/api/petugas/matrix?${geoParams()}`, {
                headers: H,
            });
            const d = await r.json();
...
watch([filterKec, filterDes, filterSls, filterSubsls], () =>
...
const turnaroundChart = computed(() => ({
    ...
    xaxis: {
        categories: turnaroundData.value
            .slice(0, 20)
            .map((r) => r.nama ?? r.uid),
    ...
    yaxis: {
        labels: {
            style: { colors: chartColor.value, fontSize: '11px' },
            maxWidth: 160,
        },
    },
...
const matrixChartOptions = computed(() => ({
    ...
    xaxis: {
        title: {
            text: 'Rata-rata waktu submit (menit) — lebih kecil lebih cepat',
            style: {
        ...
        labels: {
            formatter: (v: number) => fmtMin(v),
            style: { colors: isDark.value ? '#a1a1aa' : '#71717a' },
        },
    },
    yaxis: {
        title: {
            text: '% di-reject pengawas — lebih kecil lebih baik',
            style: {
                color: isDark.value ? '#a1a1aa' : '#71717a',
                fontSize: '11px',
            },
...
const gelombangSeries = computed(() => [
    {
        name: 'Progress %',
        data: gelombangList.value.map((r) => r.progress_pct),
    },
    {
        name: 'Approved %',
        data: gelombangList.value.map((r) => r.approved_pct),
    },
]);
</script>
    ...
    <div v-else class="space-y-3 p-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            >
                ...
                <option v-for="k in kecOptions" :key="k.code" :value="k.code">
                    {{ k.name }}
                </option>
            </select>
            <select
            ...
            >
                ...
                <option v-for="d in desOptions" :key="d.code" :value="d.code">
                    {{ d.name }}
                </option>
            </select>
            <select
            ...
            >
                ...
                <option v-for="s in slsOptions" :key="s.code" :value="s.code">
                    {{ s.name }}
                </option>
            </select>
            <select
        ...
        <template v-else-if="activeTab === 'pencacah'">
            <div class="flex items-center justify-between">
                <span class="text-xs text-muted-foreground"
                    >{{ pencacahAll.length }} pencacah</span
                >
            ...
            <div class="overflow-x-auto rounded-lg border">
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-border">
                        <tr
                            v-for="(row, i) in paginatedPencacah"
                            :key="row.uid"
                            class="hover:bg-muted/30"
                        >
                            <td class="px-3 py-2 text-muted-foreground">
                                {{ (pencacahPage - 1) * pencacahPer + i + 1 }}
                            </td>
                            <td class="px-3 py-2">
                            ...
                            <td class="px-3 py-2 text-right font-medium">
                                {{ row.total.toLocaleString('id') }}
                            </td>
                            <td
                                class="px-3 py-2 text-right text-muted-foreground"
                            >
                                {{ row.draft.toLocaleString('id') }}
                            </td>
                            <td
                                class="px-3 py-2 text-right text-blue-600 dark:text-blue-400"
                            >
                                {{ row.submitted.toLocaleString('id') }}
                            </td>
                            <td
                                class="px-3 py-2 text-right text-emerald-600 dark:text-emerald-400"
                            >
                                {{ row.approved.toLocaleString('id') }}
                            </td>
                            <td
                                class="px-3 py-2 text-right text-red-600 dark:text-red-400"
                            >
                                {{ row.rejected.toLocaleString('id') }}
                            </td>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    >
                                        <div
                                            ...
                                            :style="{
                                                width: `${Math.min(100, row.progress_pct)}%`,
                                            }"
                                        />
                                    </div>
            ...
            <div
                v-if="pencacahPages > 1"
                class="flex items-center justify-between text-xs text-muted-foreground"
            >
                <span
                    >{{
                        ((pencacahPage - 1) * pencacahPer + 1).toLocaleString(
                            'id',
                        )
                    }}–{{
                        Math.min(
                            pencacahPage * pencacahPer,
                            pencacahAll.length,
                        ).toLocaleString('id')
                    }}
                    dari {{ pencacahAll.length.toLocaleString('id') }}</span
                >
                <div class="flex gap-1">
                    <button
                        :disabled="pencacahPage === 1"
                    ...
                    <span class="px-2 py-1"
                        >{{ pencacahPage }}/{{ pencacahPages }}</span
                    >
                    <button
                        :disabled="pencacahPage >= pencacahPages"
                        class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40"
                        @click="pencacahPage++"
                    >
                        ›
        ...
        <template v-else-if="activeTab === 'turnaround'">
            ...
            <div v-if="turnaroundData.length" class="rounded-lg border p-3">
                <p class="mb-2 text-xs text-muted-foreground">
                    Menampilkan 20 tercepat di chart. Tabel menampilkan semua.
                </p>
                <VueApexCharts
                    type="bar"
                    :height="
                        Math.max(200, turnaroundData.slice(0, 20).length * 28)
                    "
                    :options="turnaroundChart"
                    :series="turnaroundSeries"
                />
            </div>
            <div class="flex items-center justify-between">
                <span class="text-xs text-muted-foreground"
                    >{{ turnaroundData.length }} entri</span
                >
                <select
                    v-model="turnaroundPer"
                    class="h-7 rounded border border-input bg-background px-1 text-xs"
                    @change="turnaroundPage = 1"
                >
                    <option :value="10">10</option>
                    <option :value="20">20</option>
                    <option :value="50">50</option>
            ...
            <div class="overflow-x-auto rounded-lg border">
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-border">
                        <tr v-if="turnaroundData.length === 0">
                            <td
                                colspan="5"
                                class="py-8 text-center text-muted-foreground"
                            >
                        ...
                        <template v-else-if="turnaroundView === 'pencacah'">
                            <tr
                                v-for="r in paginatedTurnaround as TurnaroundPencacah[]"
                                :key="r.uid"
                                class="hover:bg-muted/30"
                            >
                                <td class="px-3 py-2">{{ r.nama ?? r.uid }}</td>
                                <td class="px-3 py-2 text-right font-medium">
                                    {{ fmtMin(r.avg_minutes) }}
                                </td>
                                <td
                                    class="px-3 py-2 text-right text-muted-foreground"
                                >
                        ...
                        <template v-else>
                            <tr
                                v-for="r in paginatedTurnaround as TurnaroundPengawas[]"
                                :key="r.uid"
                                class="hover:bg-muted/30"
                            >
                                <td class="px-3 py-2">{{ r.nama ?? r.uid }}</td>
            ...
            <div
                v-if="turnaroundPages > 1"
                class="flex items-center justify-between text-xs text-muted-foreground"
            >
                <span
                    >{{ (turnaroundPage - 1) * turnaroundPer + 1 }}–{{
                        Math.min(
                            turnaroundPage * turnaroundPer,
                            turnaroundData.length,
                        )
                    }}
                    dari {{ turnaroundData.length }}</span
                >
                <div class="flex gap-1">
                    <button
                        :disabled="turnaroundPage === 1"
                        class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40"
                        @click="turnaroundPage--"
                    >
                        ‹
                    </button>
                    <span class="px-2 py-1"
                        >{{ turnaroundPage }}/{{ turnaroundPages }}</span
                    >
                    <button
                        :disabled="turnaroundPage >= turnaroundPages"
                        class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40"
                        @click="turnaroundPage++"
                    >
                        ›
        ...
        <template v-else-if="activeTab === 'quality'">
            <div class="flex items-center justify-between">
                <p class="text-xs text-muted-foreground">
                    ...
                    <code class="rounded bg-muted px-1">sum_remark</code>.
                </p>
                <select
                    v-model="qualityPer"
                    class="h-7 rounded border border-input bg-background px-1 text-xs"
                    @change="qualityPage = 1"
                >
                    <option :value="10">10</option>
                    <option :value="20">20</option>
                    <option :value="50">50</option>
            ...
            <div class="overflow-x-auto rounded-lg border">
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-border">
                        <tr
                            v-for="(row, i) in paginatedQuality"
                            :key="row.uid"
                            class="hover:bg-muted/30"
                        >
                            <td class="px-3 py-2 text-muted-foreground">
                                {{ (qualityPage - 1) * qualityPer + i + 1 }}
                            </td>
                            <td class="px-3 py-2">
                                <div class="font-medium">{{ row.nama }}</div>
                                <div class="text-[10px] text-muted-foreground">
            ...
            >
                <span
                    >{{ (qualityPage - 1) * qualityPer + 1 }}–{{
                        Math.min(qualityPage * qualityPer, qualityAll.length)
                    }}
                    dari {{ qualityAll.length }}</span
                >
                <div class="flex gap-1">
                    <button
                    ...
                    <span class="px-2 py-1"
                        >{{ qualityPage }}/{{ qualityPages }}</span
                    >
                    <button
                        :disabled="qualityPage >= qualityPages"
                        class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40"
                        @click="qualityPage++"
                    >
                        ›
                ...
                <table class="w-full text-xs">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        >
                            ...
                            <td class="px-3 py-2 text-right">
                                {{ row.total_assignment.toLocaleString('id') }}
                            </td>
                            <td
                                class="px-3 py-2 text-right text-blue-600 dark:text-blue-400"
                            >
        ...
        <template v-else-if="activeTab === 'matrix'">
            <p class="text-xs text-muted-foreground">
                Scatter plot kecepatan vs akurasi per pencacah (min 3 submit).
                Titik kiri-bawah = ideal.
            </p>
            <div
                v-if="!matrixData.length"
                class="flex h-48 items-center justify-center text-sm text-muted-foreground"
            >
                Tidak ada data (pencacah perlu ≥ 3 submit).
            </div>
            <div v-else class="rounded-lg border p-3">
                <div class="mb-3 grid grid-cols-2 gap-2 text-xs">
                    <div
                        class="rounded bg-emerald-50 px-2 py-1 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400"
                    >
                        Kiri-bawah: Cepat + Akurat (top performer)
                    </div>
                    <div
                        class="rounded bg-amber-50 px-2 py-1 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400"
                    >
                        Kanan-bawah: Cepat tapi banyak error (perlu pembinaan)
                    </div>
                    <div
                        class="rounded bg-blue-50 px-2 py-1 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400"
                    >
                        Kiri-atas: Akurat tapi lambat (perlu motivasi)
                    </div>
                    <div
                        class="rounded bg-red-50 px-2 py-1 text-red-700 dark:bg-red-900/20 dark:text-red-400"
                    >
                        Kanan-atas: Lambat + banyak error (perlu bimbingan)
                    </div>
                </div>
                <VueApexCharts
                    type="scatter"
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground"
                        >{{
                            matrixData.length.toLocaleString('id')
                        }}
                        pencacah</span
                    >
                    <select
                ...
                >
                    <span
                        >{{
                            ((matrixPage - 1) * matrixPer + 1).toLocaleString(
                                'id',
                            )
                        }}–{{
                            Math.min(
                                matrixPage * matrixPer,
                                matrixData.length,
                            ).toLocaleString('id')
                        }}
                        dari {{ matrixData.length.toLocaleString('id') }}</span
                    >
                    <div class="flex items-center gap-1">
                        ...
                        <span class="px-2 py-1"
                            >{{ matrixPage }}/{{ matrixPages }}</span
                        >
                        <button
                            :disabled="matrixPage >= matrixPages"
                            class="rounded border px-2 py-1 hover:bg-muted/40 disabled:opacity-40"
                            @click="matrixPage++"
                        >
                            ›
        ...
        <template v-else-if="activeTab === 'leaderboard'">
            <p class="text-xs text-muted-foreground">
                Skor = Progress% × 50% + (100 − Reject%) × 30% + Approved% × 20%
            </p>
            <div
            ...
            <div v-else class="space-y-2">
                <div class="flex items-center justify-between">
                    <span class="text-xs text-muted-foreground"
                        >{{
                            leaderboardData.length.toLocaleString('id')
                        }}
                        petugas</span
                    >
                    <select
                        v-model="leaderboardPer"
                        class="h-7 rounded border border-input bg-background px-1 text-xs"
                        @change="leaderboardPage = 1"
                    >
                        <option :value="10">10/hal</option>
                        <option :value="20">20/hal</option>
                        <option :value="50">50/hal</option>
                ...
                <div class="overflow-x-auto rounded-lg border">
                    <table class="w-full text-xs">
                        ...
                        <tbody class="divide-y divide-border">
                            <tr
                                v-for="r in paginatedLeaderboard"
                                :key="r.uid"
                                class="hover:bg-muted/30"
                            >
                                ...
                                <td class="px-3 py-2 text-right">
                                    {{ r.total.toLocaleString('id') }}
                                </td>
                                <td
                                    class="px-3 py-2 text-right text-emerald-600 dark:text-emerald-400"
                                >
                                    {{ r.approved.toLocaleString('id') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div
                    v-if="leaderboardPages > 1"
                    class="flex items-center justify-between text-xs text-muted-foreground"
                >
                    <span
                        >{{
                            (
                                (leaderboardPage - 1) * leaderboardPer +
                                1
                            ).toLocaleString('id')
                        }}–{{
                            Math.min(
                                leaderboardPage * leaderboardPer,
                                leaderboardData.length,
                            ).toLocaleString('id')
                        }}
                        dari
                        {{ leaderboardData.length.toLocaleString('id') }}</span
                    >
                    <div class="flex items-center gap-1">
                        <button
                        ...
                        <span class="px-2 py-1"
                            >{{ leaderboardPage }}/{{ leaderboardPages }}</span
                        >
                        <button
